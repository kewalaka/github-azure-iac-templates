...
1. **Add Required Variables:** Add the following **Secrets** to **both** the `_plan` and `_apply` environments you just created:
...
| :------------ | :---------- | :------ |
| `TF_STATE_SUBSCRIPTION_ID`      | Subscription ID for the Terraform state storage, only required if it is not the same as the deployment subscription account.   | `secrets.ARM_SUBSCRIPTION_ID` |
